...
1) Replaced "querySelector" by "getElementById" --> Lines#: 404/407/410/423/470/558
2) Replaced "querySelectorAll" by "getElementsByClassName" --> Lines#: 450/506
3) Moved all variables outside the for loop and created variable "allPizzas" to apply newWidth and dx to all pizza containers --> #: 448-457
4) Moved variables pizzasDiv outside the for loop --> #: 470-475
5) Created a function for caching items & returning cached items if they are cached --> #: 501-520
6) Added "createDocumentFragment" to create a node not included in the DOM and then attached to the DOM --> #: 536-560
...
var resizePizzas = function(size) {
  ...
  /* 1) Replaced querySelector by getElementById */
  ...
  /* 2-3) Replaced "querySelectorAll" by "getElementsByClassName". Created new variable allPizzas used to iterate through, with size based on dx and newwidth variables. Moved variables outside of for loop. */
  function changePizzaSizes(size) {
    var allPizzas = document.getElementsByClassName("randomPizzaContainer");
    var dx = determineDx(allPizzas[0], size);
    var newwidth = (allPizzas[0].offsetWidth + dx) + 'px';
    for (var i = 0, l= allPizzas.length; i < l; i++) {
  ...
  console.log("Time to resize pizzas: " + timeToResize[0].duration + "ms"); // replaced 0
};

window.performance.mark("mark_start_generating"); // collect timing data

// This for-loop actually creates and appends all of the pizzas when the page loads
/* 4) Moved variable "pizzasDiv" outside the for loop; replaced "querySelector" by "getElementById"; created the variable "pizzaFragment" equal to document.createDocumentFragment to create a node outside the DOM */
...
/* 5) Created function "getItems" for caching items & returning cached items */
...
/* 6) Added a createDocumentFragment, it helps performance. The number of pizzas is now based on the screen size (the variable maxPizzas reflects
  this number) */
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var cols = Math.round(screenWidth/150);
  var rows = Math.round(screenHeight/150);
  ...
  for (var i = 0; i < maxPizzas; i++) {
    ...
    backFragment.appendChild(elem);
  }
  document.getElementById("movingPizzas1").appendChild(backFragment);
  updatePositions();
});